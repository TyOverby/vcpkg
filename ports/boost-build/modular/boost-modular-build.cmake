function(setup_build_jam SOURCE_DIR)
    ...
    file(INSTALL "${CURRENT_INSTALLED_DIR}/tools/boost-build/boostcpp.jam"
        ...
        file(INSTALL "${CURRENT_INSTALLED_DIR}/share/boost_install"
...
function(boost_modular_build)
    ...
    if(EXISTS "${BOOST_BUILD_PATH}/b2${VCPKG_HOST_EXECUTABLE_SUFFIX}")
        set(B2_EXE "${BOOST_BUILD_PATH}/b2${VCPKG_HOST_EXECUTABLE_SUFFIX}")
    else()
    ...
    if(VCPKG_TARGET_IS_WINDOWS)
        ...
        set(BOOST_LIB_RELEASE_SUFFIX -${VCPKG_PLATFORM_TOOLSET}.lib) #TODO: check suffix
        set(BOOST_LIB_DEBUG_SUFFIX -${VCPKG_PLATFORM_TOOLSET}-gd.lib)
    else()
    ...
    #configure_file(${_bm_DIR}/Jamroot.jam.in ${_bm_SOURCE_PATH}/Jamroot.jam @ONLY)
    ...
    configure_file("${_bm_DIR}/user-config.jam.in" ${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/user-config.jam @ONLY)
    configure_file("${_bm_DIR}/user-config.jam.in" ${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/user-config.jam @ONLY)
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
        ...
        file(TO_NATIVE_PATH "${CURRENT_PACKAGES_DIR}/test" INSTALL_PREFIX_PATH )
        file(TO_NATIVE_PATH "${CURRENT_PACKAGES_DIR}/test/tools/${PORT}" INSTALL_TOOLS_PATH )
        vcpkg_execute_required_process(
            COMMAND "${B2_EXE}"
                "install"
                "--prefix=${INSTALL_PREFIX_PATH}"
                "--bindir=${INSTALL_TOOLS_PATH}"
                "--stagedir=${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/stage"
                "--build-dir=${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel"
                "--user-config=${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/user-config.jam"
                "${_bm_OPTIONS}"
