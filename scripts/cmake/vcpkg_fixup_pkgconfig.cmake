    ...
    foreach(_lib IN LISTS _pkg_libs_output)
        ...
        elseif (_lib MATCHES "^-l(.+)$")
            ...
            foreach(_lib_suffix ${lib_suffixes})
                ...
                find_library(CHECK_LIB_${_libname}_${_config} NAMES ${_name_without_extension}${_lib_suffix} "${CURRENT_INSTALLED_DIR}${PATH_SUFFIX_${_config}}/lib" PATHS ${_pkg_lib_paths_output})
                if(CHECK_LIB_${_libname}_${_config})
                    message(FATAL_ERROR "Found ${CHECK_LIB_${_libname}_${_config}} with additional debug suffix! Please correct the *.pc file!")
                endif()
            endforeach()
            # Reaching here means error!
            message(STATUS "CHECK_LIB_${_libname}:${CHECK_LIB_${_libname}_${_config}}")
